...
class Spreadsheet::XLSX does Spreadsheet::XLSX::Root {
    ...
    #|
    #| SPIKE SEAM (Task 8): with :fast, delegate to the standalone string-based
    #| serializer instead of the DOM archive-write path. The string serializer
    ...
    method to-blob(Bool :$fast --> Blob) {
        ...
        return self!to-blob-dom;
    }

    #| Original DOM serialization body, extracted unchanged so the fast seam and
    #| the dom-blob shim can both reach it.
    method !to-blob-dom(--> Blob) {
